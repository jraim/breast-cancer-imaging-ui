...
const ellipseInitPosition = {
    x: 50,
    y: 50,
};

const ellipseInitFormat = {
    height: 80,
    width: 50,
    rotation: 45,
};
...
export default class DicomStudio extends Component {
    constructor(props) {
        ...
        this.state = {
            ...
            totalNumSlices: null,
            slices: null,
            ePosition: ellipseInitPosition,
            eFormat: ellipseInitFormat,
        };
    }
    ...
    toolBoxActions = {
        ...
        submitSelection: () => {
            console.log(this.state.ePosition);
            console.log(this.state.eFormat);
        },
    };
    ...
    eventHandlers = {
        onDragStart: (evt) => {},
        onDragEnd: (evt) => {
            this.setState({ ePosition: { x: evt.target.x(), y: evt.target.y() } });
        },
    };
                ...
                <div className='cui flex column ai-center jc-center' style={styles.stage}>
                    ...
                    <div className='cui flex row ai-center jc-center' style={styles.imageBox}>
                        {!this.state.curSlice ? (
                            ...
                            <Stage width={600} height={400} onDragEnd={this.eventHandlers.onDragEnd}>
                                <Layer>
                                    <URLImage src={this.state.curSliceUrl} />
                                    <MyEllipse format={this.state.eFormat} position={this.state.ePosition} eventHandlers={this.eventHandlers} />
                                </Layer>
                            </Stage>
                        )}
                    </div>
